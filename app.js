...
function loadData() {
  try {
    return Object.assign({}, emptyData, JSON.parse(localStorage.getItem(KEY)) || {});
  } catch (e) {
    return { ...emptyData };
  }
}
...
function renderCalendar() {
  ...
  for (let d = 1; d <= days; d++) {
    ...
    const today = date === isoDate(new Date()) ? "today" : "";
...
function renderDay() {
  ...
  document.getElementById("dayAppointments").innerHTML = `<h3>${fmtDate(selectedDate)}</h3>` +
    (list.length ? list.map(a => {
      let c = data.customers.find(x => x.id === a.customerId);
      return `<div class="appointment"><div><div class="time">${esc(a.time || "--:--")}</div><strong>${esc(c?.name || a.customerName || "Müşteri silinmiş")}</strong><br><small>${esc(a.jobType || "İş belirtilmedi")} ${a.address ? `· ${esc(a.address)}` : ""}</small></div><div class="appointment-actions"><button onclick="editAppointment('${a.id}')">Düzenle</button><button onclick="deleteAppointment('${a.id}')">Sil</button></div></div>`;
    }).join("") : `<div class="empty">Bu gün için randevu yok.</div>`);
}
...
function appointmentForm(date, id = null) {
  ...
  document.getElementById("appointmentForm").onsubmit = e => {
    e.preventDefault();
    let f = new FormData(e.target), parts = readPicker();
    ...
    if (id) data.appointments = data.appointments.map(x => x.id === id ? obj : x);
    else data.appointments.push(obj);
    save();
    closeModal();
...
function partPickerHtml(selected) {
  return `<div>${data.parts.length ? data.parts.map(p => {
    let old = selected.find(x => x.partId === p.id);
    return `<div class="part-row"><label><input type="checkbox" data-part="${p.id}" ${old ? "checked" : ""}> ${esc(p.name)}<br><small>${money(p.price)}</small></label><input type="number" min="1" value="${old?.qty || 1}" data-qty="${p.id}"><button type="button" onclick="this.parentElement.remove()">×</button></div>`;
  }).join("") : `<span class="muted-text">Henüz parça yok. Önce “Parçalarım” bölümünden ekle.</span>`}</div>`;
}

function readPicker() {
  return [...document.querySelectorAll("#picker input[type=checkbox]:checked")].map(ch => {
    let p = data.parts.find(x => x.id === ch.dataset.part), q = Number(document.querySelector(`[data-qty="${ch.dataset.part}"]`)?.value || 1);
    return { partId: p.id, name: p.name, price: Number(p.price), qty: q };
  });
}
...
function customerForm(id = null) {
  ...
  document.getElementById("customerForm").onsubmit = e => {
    e.preventDefault();
    let f = new FormData(e.target), obj = { id: id || uid(), name: f.get("name"), phone: f.get("phone"), address: f.get("address"), note: f.get("note") };
    if (id) data.customers = data.customers.map(x => x.id === id ? obj : x);
    else data.customers.push(obj);
    save();
    closeModal();
    renderCustomers();
...
function renderCustomers() {
  ...
  document.getElementById("customerList").innerHTML = arr.length ? arr.map(c => {
    let count = data.appointments.filter(a => a.customerId === c.id).length;
    return `<div class="card"><h3>${esc(c.name)}</h3><p>${esc(c.phone || "Telefon yok")}</p><p>${esc(c.address || "Adres yok")}</p><p>${count} randevu</p><div class="actions"><button onclick="customerDetails('${c.id}')">Detay</button><button onclick="customerForm('${c.id}')">Düzenle</button><button onclick="deleteCustomer('${c.id}')">Sil</button></div></div>`;
  }).join("") : `<div class="empty">Henüz müşteri yok.</div>`;
}

function customerDetails(id) {
  let c = data.customers.find(x => x.id === id), aps = data.appointments.filter(a => a.customerId === id).sort((a, b) => b.date.localeCompare(a.date));
  openModal(c.name, `<p><b>Telefon:</b> ${esc(c.phone || "-")}<br><b>Adres:</b> ${esc(c.address || "-")}<br><b>Not:</b> ${esc(c.note || "-")}</p><h4>Geçmiş / Randevular</h4>${aps.length ? aps.map(a => `<div class="card"><b>${fmtDate(a.date)} ${esc(a.time || "")}</b><p>${esc(a.jobType || "")}</p><p>İşçilik: ${money(a.labor)} · Parçalar: ${money(a.parts.reduce((s, p) => s + Number(p.price) * Number(p.qty), 0))}</p></div>`).join("") : `<div class="empty">Randevu yok.</div>`}`);
}

function deleteCustomer(id) {
  if (confirm("Müşteri silinsin mi? Randevuları da silinir.")) {
    data.customers = data.customers.filter(x => x.id !== id);
    data.appointments = data.appointments.filter(x => x.customerId !== id);
    save();
    renderCustomers();
    renderCalendar();
...
function partForm(id = null) {
  ...
  document.getElementById("partForm").onsubmit = e => {
    e.preventDefault();
    let f = new FormData(e.target), obj = { id: id || uid(), name: f.get("name"), price: Number(f.get("price") || 0) };
    if (id) data.parts = data.parts.map(x => x.id === id ? obj : x);
    else data.parts.push(obj);
    save();
    closeModal();
    renderParts();
    toast("Parça kaydedildi.");
  };
}

function renderParts() {
  document.getElementById("partList").innerHTML = data.parts.length ? data.parts.map(p => `<div class="card"><h3>${esc(p.name)}</h3><p><b>${money(p.price)}</b></p><div class="actions"><button onclick="partForm('${p.id}')">Düzenle</button><button onclick="deletePart('${p.id}')">Sil</button></div></div>`).join("") : `<div class="empty">Henüz parça eklemedin.</div>`;
}

function deletePart(id) {
  if (confirm("Bu parça silinsin mi? Eski randevulardaki kayıtlar korunur.")) {
...
// Bütün HTML DOM elemanları yüklendikten sonra kodları çalıştır
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("exportBtn").onclick = () => {
    ...
    a.download = `kombi-ustasi-yedek-${isoDate(new Date())}.json`;
    a.click();
  ...
  document.getElementById("importInput").onchange = e => {
    ...
    r.onload = () => {
      try {
        ...
        if (!x.customers || !x.parts || !x.appointments) throw 0;
        data = x;
        save();
        renderCalendar();
        renderCustomers();
  ...
  document.getElementById("clearBtn").onclick = () => {
    if (confirm("TÜM müşteri, randevu ve parça kayıtları silinecek. Emin misin?")) {
      data = { ...emptyData };
      save();
      renderCalendar();
      renderCustomers();
